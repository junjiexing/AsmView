...
//	
LONG AsmView::OnMouseActivate(HWND hwndTop, UINT nHitTest, UINT nMessage)
{
	SetFocus(m_hWnd);
	return MA_ACTIVATE;
}
...
//
LONG AsmView::OnLButtonDown(UINT nFlags, int mx, int my)
{
	ULONG nLineNo;
	ULONG nCharOff;
	ULONG nFileOff;
...
//
LONG AsmView::OnLButtonUp(UINT nFlags, int mx, int my)
{
...
//
LONG AsmView::OnMouseMove(UINT nFlags, int mx, int my)
{
	if(m_nSelectionMode)
	{
		...
		//	is inside/outside of the AsmView scrolling area
...
//
LONG AsmView::OnTimer(UINT nTimerId)
{
...
//
BOOL AsmView::MouseCoordToFilePos(	int		 mx,			// [in]  mouse x-coord
...
LONG AsmView::InvalidateLine(ULONG nLineNo)
{
...
//
LONG AsmView::InvalidateRange(ULONG nStart, ULONG nFinish)
{
...
//
VOID AsmView::MoveCaret(int x, int y)
{
...
void AsmView::UpdateLine(ULONG nLineNo)
{
	// redraw the old and new lines if they are different
