...
//
VOID AsmView::RefreshWindow()
{
	InvalidateRect(m_hWnd, NULL, FALSE);
}

//
//	Painting procedure for AsmView objects
//
LONG AsmView::OnPaint()
{
	PAINTSTRUCT ps;
...
//
void AsmView::PaintLine(HDC hdc, ULONG nLineNo)
{
	RECT  rect;
...
//
void AsmView::UpdateMarginWidth()
{
...
//
int AsmView::PaintMargin(HDC hdc, ULONG nLineNo, RECT *margin)
{
	RECT  rect = *margin;
	TCHAR ach[32];
...
//
//	Draw a line of text into the AsmView window
//
void AsmView::PaintText(HDC hdc, ULONG nLineNo, RECT *rect)
{
...
//
int AsmView::ApplyTextAttributes(ULONG nLineNo, ULONG nOffset, ULONG &nColumn, TCHAR *szText, int nTextLen, ATTR *attr)
{
...
//	
int AsmView::NeatTextOut(HDC hdc, int xpos, int ypos, TCHAR *szText, int nLen, int nTabOrigin, ATTR *attr)
{
	int   i;
	int   xold = xpos;
	int   lasti = 0;
...
//
//
COLORREF AsmView::LineColour(ULONG nLineNo)
{
...
COLORREF AsmView::LongColour(ULONG nLineNo)
{
	if(m_nCurrentLine == nLineNo && CheckStyle(TXS_HIGHLIGHTCURLINE))
		return GetColour(TXC_CURRENTLINE);
	else
...
//
COLORREF AsmView::GetColour(UINT idx)
{
...
COLORREF AsmView::SetColour(UINT idx, COLORREF rgbColour)
{
	COLORREF rgbOld;
